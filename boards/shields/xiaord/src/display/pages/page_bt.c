...
LV_FONT_DECLARE(symbol_custom)

/* ── Profile count ──────────────────────────────────────────────────────── */

#if IS_ENABLED(CONFIG_ZMK_BLE)
    #define BT_PROFILE_COUNT_RAW (CONFIG_BT_MAX_CONN - CONFIG_ZMK_SPLIT_BLE_CENTRAL_PERIPHERALS)
...
static void bt_endpoint_cb(struct endpoint_state state) {
    ...
    for (int i = 0; i < BT_PROFILE_COUNT; i++) {
        lv_obj_clear_state(s_profile_btns[i], LV_STATE_CHECKED | LV_STATE_USER_1 | LV_STATE_USER_2 | LV_STATE_USER_3);
        ...
        LOG_DBG("profile[%d]: prefer_bt=%d selected=%d bonded=%d connected=%d -> %s",
                ...
                bonded,
                connected,
                prefer_bt ? (selected ? (bonded && connected ? "blue"
                                         : bonded            ? "red"
                                                             : "yellow")
                                      : (bonded ? "green" : "white"))
                          : (bonded     ? "green"
...
static int page_bt_create(lv_obj_t *screen) {
    ...
    for (int i = 0; i < BT_PROFILE_COUNT; i++) {
        ...
        lv_obj_t *btn     = ui_create_circle_btn(screen, label, pos[slot][0], pos[slot][1], profile_btn_cb, (void *)(uintptr_t)i);
        s_profile_btns[i] = btn;
    }
    /* Output status label — upper inner area, large font */
    s_output_lbl = create_output_status_label(screen, &symbol_custom);
