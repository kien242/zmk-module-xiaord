...
LV_FONT_DECLARE(montserrat_regular_48_custom)
...
static void update_datetime(lv_timer_t *t) {
    ...
    /* "Jan 01 Mon" */
    lv_label_set_text_fmt(s_date_lbl, "%s %02d %s", month_names[time.tm_mon], time.tm_mday, day_names[time.tm_wday]);

    /* "23:59" */
    lv_label_set_text_fmt(s_time_lbl, "%02d:%02d", time.tm_hour, time.tm_min);
}
...
static int page_home_create(lv_obj_t *tile) {
    ...
    lv_obj_set_style_text_font(s_time_lbl, &montserrat_regular_48_custom, 0);
